/**
 * Description: A bunch of methods to perform conversion
 *              of byte arrays from one encoding to another 
 *              and/or toString.
...
public class UtilsConverter implements UnfCons{
	...
	UnsupportedEncodingException{
       //default Charset in JVM
			...
			if(Charset.isSupported(cset[ORG_ENC]) ){
				...
				if(from.canEncode())
			        strbin = new String(bin,cset[ORG_ENC]);
				     ...
				     strbin =  new String(bin,textencoding);	
				} 
		    }else{	
		...
		byte[] bout = strbin.getBytes(cset[FINAL_ENC]);
		return bout;
	}
	/**
	  * Convert byte[] to byte[] different Encodings
